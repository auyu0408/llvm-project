...
PreservedAnalyses MyPass2::run(Function &F, FunctionAnalysisManager &AM){
    ...
    if(!inlined_flag) return PreservedAnalyses::none();
...
std::unordered_map<std::pair<Value *, Value *>, int, PairHash> buildCapacity2(Function &F){
    ...
    for(int i = 0; i < total - 1; i++){
        int center = (total + 1) / 2;
        int dist = std::abs(center - (i+1))+1;
        addDependency(InstLists[i], InstLists[i+1], (CutCounts[i] + ValueCounts[i]) * dist, Cap);
    }

    if(F.getReturnType()->isVoidTy()){
        if(total-2 >= 0){
            int center = (total + 1) / 2;
            addDependency(InstLists[total-2], InstLists[total-1], center, Cap); // 如果是void function，最後一個指令會是return void
        }
    }

    /*** debug output
    for(auto ele:Cap){
        Value *src = ele.first.first;
        Value *dest = ele.first.second;
        int weight = ele.second;

        errs() << "Edge: " << *src << " -> " << *dest << " with weight: " << weight << "\n";
    }
    ***/
